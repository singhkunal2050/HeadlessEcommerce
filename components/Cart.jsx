...
function Cart() {
  ...
  return (
    <>
        ...
        <div className="cart-container overflow-y-auto transition-all duration-300 ease-in-out z-10 px-4 fixed right-0 top-0 bottom-0 w-full  md:w-[400px] min-h-screen bg-white" style={{ transform: cartVisibility ? 'translateX(0)' : 'translateX(110%)' }}>
              ...
              return(
                <div key={item.variantId} className="cart-item flex border-2 shadow-sm">
                  <div className="cart-item-img flex ">
                    <Image className="h-[100%] w-full aspect-square object-cover" src={item.customAttributes[2].value} alt={item.customAttributes[0].value}  width="100px" height="100px" />
                  </div>
                  <div className="cart-item-details p-4  pt-1 flex flex-col">
                    <h4 className="text-darknight font-bold flex-1" >{item.customAttributes[0].value}</h4>
                    <p>{item.quantity}</p>
                    <p className="font-extralight text-gray-800">${item.customAttributes[1].value}</p>
                  </div>
                </div>
              )
